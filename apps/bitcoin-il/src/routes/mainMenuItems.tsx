...
export interface MenuItemBeingPrepared {
  label: JSX.Element
  key: string
  children?: any[]
}

const stripItemsFromArrayOfMainMenuItems = (menuArray: MainMenuItem[]) => {
  console.log('STRIPPING')
  console.log(menuArray)
  const newArray: ItemType[] = [] // init var

  menuArray.forEach((menuItem) => {
    const toBePushedToNewArray: MenuItemBeingPrepared = {
      label: (
        <MenuItem
          label={
            <CustomNavLink to={`/${menuItem.key}`}>
              {menuItem.label}
            </CustomNavLink>
          }
        />
    ...
    if (!!menuItem.submenu)
      toBePushedToNewArray.children = stripItemsFromArrayOfMainMenuItems(
        menuItem.submenu
      )

    newArray.push(toBePushedToNewArray)
  })

  return newArray
}

export const recursiveGenerateMenuItems = (newItems?: MainMenuItem[]) => {
  const menuAsAntdItems: any[] = [] // init var

  if (!newItems) {
    menuAsAntdItems.push(stripItemsFromArrayOfMainMenuItems(mainMenuItems))
  } else {
    menuAsAntdItems.push(stripItemsFromArrayOfMainMenuItems(newItems))
  }

  return menuAsAntdItems
}
